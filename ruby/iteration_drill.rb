...
# ----
def check_supplies(arr, item)
p arr.include?(item)
end

check_supplies(zombie_apocalypse_supplies, "boots")
check_supplies(zombie_apocalypse_supplies, "shotgun")
# 3. You can't carry too many things, you've only got room in your pack for 5.
# Remove items in your zombie_apocalypse_supplies in any way you'd like,
# leaving only 5, using #each.
# ----
p zombie_apocalypse_supplies.each.take(5)
...
new_supplies = (arr1 << arr2).flatten
new_supplies = new_supplies.uniq
p new_supplies
end
...
# ----
def remove_less_2k(hsh)
hsh.delete_if{ |key, value| value > 2000}
p hsh	
end
remove_less_2k(extinct_animals)
# 3. Our calculations were completely off, turns out all of those animals went
# extinct 3 years before the date provided. Update the values in extinct_animals
# using #each, so they accurately reflect what year the animal went extinct.
# ----
def year_minus_3(hsh)
  hsh.each{ |key, value| hsh[key] -= 3 }
  p hsh
end
year_minus_3(extinct_animals)
...
# ----
def not_extinct(hsh, animal)
    hsh.delete(animal)
    print hsh
end
not_extinct(extinct_animals, "Passenger Pigeon")
